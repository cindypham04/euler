...
Usage:
    ...
    Result appears on phone and laptop terminal.
...
try:
    ...
    sys.exit("Run:  pip install aiohttp")
...
def _preprocess(bgr):
    ...
    h, w = binary.shape
    up   = cv2.resize(binary, (w * UPSCALE, h * UPSCALE), interpolation=cv2.INTER_CUBIC)
...
def _run_ocr(model, *, crop: bool) -> str | None:
    data = BUFFER.get()
    if data is None:
    ...
    if frame is None:
        ...
        return None
    region    = _crop_center(frame) if crop else frame
    processed = _preprocess(region)
    print("Recognizing…", flush=True)
    try:
        eq     = model(Image.fromarray(processed)).strip()
        ts     = datetime.now().strftime("%H:%M:%S")
        result = eq or "(no equation detected)"
...
def main() -> int:
    ...
    print("Loading pix2tex model…", flush=True)
    from pix2tex.cli import LatexOCR
    model = LatexOCR()
    print("Model ready.  Waiting for phone stream…\n", flush=True)
